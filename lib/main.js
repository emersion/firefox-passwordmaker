...
function getPasswordMakerDeps() {
  return [
    ...
    // self.data.url("js/password-auto-fill.js")
...
if (!isOnMobile()) {
  ...
  function getPanel(){
    if (panel==undefined) {
      ...
      panel.port.on("auto_fill_password", function (data) {
        destroyPanel();
        clipboard.set(data.passwd);
        if (tabWorkers[tabs.activeTab.id]!=undefined) {
          tabWorkers[tabs.activeTab.id].port.emit("auto_fill_password",data.passwd);
        }
      });
  ...
  function destroyPanel(){
    getPanel().destroy();       // after panel is destory ,web page can get focus again
    panel=undefined;
  }
  // var currentTabWorker=undefined;
  var tabWorkers={}            //key=tab.id ,value =worker
  tabs.on("ready", function(tab){
    currentTabWorker=tab.attach({
      contentScriptFile: self.data.url("js/password-auto-fill.js")
    });
    tabWorkers[tab.id]=currentTabWorker;
    currentTabWorker.port.on("after_set_focused_password_field", function () {
      getPanel().show();
    });
  });
  var showHotKey = Hotkey({
    ...
    onPress: function() {
      if (tabWorkers[tabs.activeTab.id]!=undefined) {
        tabWorkers[tabs.activeTab.id].port.emit("remember_current_focus_element");
      }
      // panel.show();
    }
  });
  ...
  var button = ToggleButton({
    ...
    label: "Password Maker("+prefs.hotKey+") accel=osx.cmd=windows.ctrl=linux.ctrl",
    icon: {
      "16": "./icon-16.png",
      "32": "./icon-32.png",
      "64": "./icon-64.png"
